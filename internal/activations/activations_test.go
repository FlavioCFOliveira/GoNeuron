...
func TestReLU(t *testing.T) {
	...
	for _, tt := range tests {
		...
		if math.Abs(output-tt.expected) > 1e-10 {
...
func TestSigmoid(t *testing.T) {
	...
	tests := []struct {
		...
		{-math.Inf(1), 0.0},    // -inf -> 0
		{-2.0, 1 / (1 + math.Exp(2))},
		{-1.0, 1 / (1 + math.Exp(1))},
		{0.0, 0.5},             // Zero -> 0.5
		{1.0, 1 / (1 + math.Exp(-1))},
		{2.0, 1 / (1 + math.Exp(-2))},
		{math.Inf(1), 1.0},     // +inf -> 1
	}

	for _, tt := range tests {
		output := sigmoid.Activate(tt.input)
		if math.Abs(output-tt.expected) > 1e-9 {
...
func TestTanhDerivative(t *testing.T) {
	...
	expected = 1 - math.Tanh(1.0)*math.Tanh(1.0)
	if math.Abs(output-expected) > 1e-10 {
...
func TestLeakyReLUContinuity(t *testing.T) {
	...
	fPlus := leakyReLU.Activate(1e-10)
	fMinus := leakyReLU.Activate(-1e-10)

	if math.Abs(f0-fPlus) > 1e-10 || math.Abs(f0-fMinus) > 1e-10 {
...
func TestActivationsAreDifferent(t *testing.T) {
	...
	inputs := []float64{-2.0, -1.0, 0.5, 1.0, 2.0}
...
func TestActivationConsistency(t *testing.T) {
	...
	relu := ReLU{}
	for _, x := range []float64{-1.0, -0.5, 0.5, 1.0} {
		...
		if math.Abs(numericDeriv-analyticDeriv) > 1e-4 {
