...
func main() {
	content, err := ioutil.ReadFile("examples/dialogue_bot/datasets/dialogue.txt")
	if err != nil {
		fmt.Printf("Error: %v\n", err)
		os.Exit(1)
	}
	...
	fmt.Println("Training Dialogue Bot...")
	startTime := time.Now()
	...
	fmt.Println("\nDialogue Simulation:")
	queries := []string{"User: Olá!\nBot:", "User: Qual é o seu nome?\nBot:", "User: Você gosta de Go?\nBot:"}
