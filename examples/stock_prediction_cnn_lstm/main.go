// Stock Prediction example using CNN-LSTM hybrid architecture.
package main
...
func createCNNLSTMNetwork(lookback int, lstmUnits int) *net.Network {
	// Conv2D configured for 1D-like convolution
	// InChannels=1, OutChannels=16, KernelSize=3
	conv := layer.NewConv2D(1, 16, 3, 1, 1, activations.ReLU{})
	conv.SetInputDimensions(1, lookback)

	flatten := layer.NewFlatten()

	// LSTM input size will be 16 * lookback (if stride=1, padding=1)
	lstm := layer.NewLSTM(16*lookback, lstmUnits)
	...
	return net.New(layers, loss.MSE{}, &opt.SGD{LearningRate: 0.005})
}

func main() {
	fmt.Println("=== Stock Price Prediction using CNN-LSTM ===")
	const (
		lookback   = 10
		lstmUnits  = 32
		epochs     = 30
	...
	data, err := loadStockData("examples/stock_prediction_cnn_lstm/datasets/GOOG.csv")
	...
	network := createCNNLSTMNetwork(lookback, lstmUnits)

	for epoch := 1; epoch <= epochs; epoch++ {
		totalLoss := 0.0
		for _, seq := range trainSeq {
			// Reset LSTM layer
			network.Layers()[2].Reset()
